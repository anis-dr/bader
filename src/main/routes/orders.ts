...
export const ordersRouter = router({
      ...
      const result = db.transaction((tx) => {
        const allOrders = tx
          .select({
            ...
            creator: {
              id: users.id,
              name: sql<string>`coalesce(${users.firstName} || ' ' || ${users.lastName}, ${users.username})`.as(
                'name'
              )
            },
            client: {
  ...
  // Create new order
  create: protectedProcedure.input(CreateOrderSchema).mutation(async ({ input }) => {
    try {
      return await db.transaction(async (tx) => {
        ...
        // Create order
        const order = await tx.insert(orders).values(input).returning().get()
    ...
    .mutation(async ({ input }) => {
      try {
        return db.transaction(async (tx) => {
          ...
          const items = tx.select().from(orderItems).where(eq(orderItems.orderId, input.id)).all()
